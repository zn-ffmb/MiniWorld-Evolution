...
请执行一级传播（仅计算 Agent 动作的直接影响）:
1. 将每个动作与关系网络匹配，确定直接影响路径
2. 对每个被直接影响的实体，更新其 status 和 tags
3. 只更新确实被动作直接影响的实体，不要处理间接连锁反应（二级传播由后续轮次处理）
...
{{
  ...
  "propagation_summary": "本 tick 一级传播效应的总结（1-2句话）"
}}
"""


# ============================================================
# WorldLLM — Step 4b: 级联传播 (v3 新增)
# ============================================================

WORLD_CASCADE_PROPAGATE_PROMPT = """你是闭合小世界的世界引擎（WorldLLM）。这是第 {round_num} 轮级联传播。
...
请判断:
1. 上一轮的状态变化是否沿关系网络触发了新的连锁反应？
2. 只输出确实被上一轮变化影响的实体，影响必须有因果逻辑
3. 如果影响已衰减到不足以改变任何实体状态，输出空列表

绝对约束:
- 不能新增或删除实体和边
- 不要重复修改前几轮已更新的实体（除非有新的因果路径）
- 级联传播的影响力应逐轮衰减
...
{{
  "entity_updates": [
    {{
      ...
      "new_status": "更新后的状态描述",
      "new_tags": {{"key": "value"}},
      "change_reason": "因为上一轮xxx的变化沿xxx关系边影响了该实体",
      "caused_by": ["上一轮变化的实体ID"]
    }}
  ],
  "cascade_exhausted": true/false,
  "cascade_reasoning": "级联传播路径描述（或耗尽原因）"
}}
"""
