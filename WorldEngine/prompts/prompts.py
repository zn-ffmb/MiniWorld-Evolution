...
### 认知层级分配（重要！）
不同类型的决策者在真实世界中的思维方式差异巨大。请为每个 **human** 类实体分配认知层级（cognition_style）:
- **"strategic"**: 拥有专业分析团队或体制化决策流程的机构（主权国家政府、央行、军方、大型企业集团、国际组织联盟）
- **"intuitive"**: 有经验但无正式分析体系的决策者（中小企业、行业协会、社区组织、意见领袖、专业人士群体）
- **"reactive"**: 群体性、情绪驱动的参与者（公众舆论、消费者群体、社交媒体情绪、散户投资者、选民群体、抗议运动）
- nature 类实体不需要此字段（留空字符串）

分配依据: 从 evidence 中该实体的描述和行为模式判断——
  - 该实体有没有系统化的决策流程？（情报机构、智库、参谋团）→ strategic
  - 该实体是凭经验/直觉做决策的？ → intuitive
  - 该实体更像一个群体的集体情绪/反应？ → reactive
...
{{
  "new_entities": [
    {{
      ...
      "cognition_style": "strategic/intuitive/reactive（仅human类需要，nature类留空字符串）"
    }}
  ],
  "updated_entities": [
    {{
...
# ============================================================
# Phase 6a-0: 利益-目标提取 (InterestExtractionNode — v3 新增)
# ============================================================

INTEREST_EXTRACTION_PROMPT = """你是一个利益分析专家。请基于以下真实资料，提取该实体的利益诉求和目标体系。

核心任务:
从 evidence 中的已发生事实中，推断该实体的利益结构——不是它"说"自己关心什么，
而是从它的"行为模式"中反推它真正在乎什么。人们的行为比言辞更诚实。

⚠️ 证据闭合约束（绝对规则）:
- 每个利益维度必须附带 supporting_evidence: 引用 evidence 中的具体行为事实
- 不允许基于实体身份/名称/常识推断利益（不能因为"它是美国"就推断"它关心石油安全"——必须从实际行为推断）
- 如果 evidence 中找不到某个利益的行为证据，则不输出该维度
- 利益的 description 必须是对 supporting_evidence 中行为事实的直接概括，不能做动机猜测
- 理性约束也必须有 evidence 支撑

分析框架:
1. 从行为中反推利益维度：该实体过去做了什么？这些行为在保护/追求什么利益？
2. 判断优先级：当多个利益冲突时，它选择了保护哪个？牺牲了哪个？
3. 判断当前满足度：根据最新状态，该利益是安全的、受威胁的、还是已经被侵犯？
4. 识别理性约束：什么因素限制了它做出"最优"决策？（仅从 evidence 中可观察到的约束）
...
输出严格JSON格式（不要 ```json 标记）:
{{
  "interests": [
    {{
      "dimension": "利益维度名",
      "description": "对行为事实的直接概括（不能做动机猜测）",
      "priority": "core/important/secondary",
      "current_satisfaction": "satisfied/threatened/under_attack",
      "supporting_evidence": ["引用的具体证据原文片段1", "证据片段2"],
      "related_entities": ["与该利益相关的实体名"]
    }}
  ],
  "goal_structure": {{
    "survival_goals": ["生存级目标1"],
    "strategic_goals": ["战略级目标1"],
    "opportunistic_goals": ["机会级目标1"],
    "rationality_constraints": "理性约束描述（必须有证据支撑）"
  }}
}}
"""
...
# ============================================================
# Phase 6a-0.5: 利益覆盖审视 (v3 新增)
# ============================================================

INTEREST_COVERAGE_REVIEW_PROMPT = """你是一个利益分析审查员。请审查以下利益提取结果是否遗漏了 evidence 中存在的重要行为模式。
...
已提取的利益维度:
{extracted_interests_summary}

请检查:
1. evidence 中是否有明显的行为模式没有被上述利益维度覆盖？
2. 是否有行为事实暗示了被遗漏的利益诉求？

如果发现遗漏，输出补充的利益维度（遵循同样的证据闭合约束）。
如果没有遗漏，输出空列表。
...
{{
  ...
  "supplementary_interests": [
    {{
      "dimension": "被遗漏的利益维度名",
      "description": "对行为事实的直接概括",
      "priority": "core/important/secondary",
      "current_satisfaction": "satisfied/threatened/under_attack",
      "supporting_evidence": ["证据原文片段"],
